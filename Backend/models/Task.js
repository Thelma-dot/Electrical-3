...
class Task {
    ...
    static async findByUser(userId) {
        const sql = `
            ...
            LEFT JOIN users ua ON ua.id = t.assigned_by
            LEFT JOIN users uu ON uu.id = t.assigned_to
            WHERE t.assigned_to = ? AND (t.hidden_from_user = 0 OR t.hidden_from_user IS NULL)
    ...
    static async findCompletedByUser(userId) {
        const sql = `
            ...
            FROM tasks t
            LEFT JOIN users ua ON ua.id = t.assigned_by
            LEFT JOIN users uu ON ua.id = t.assigned_to
        ...
        try {
            ...
            throw new Error(`Failed to find completed user tasks: ${error.message}`);
        }
    }
    ...
    static async findAllForAdmin() {
        ...
        try {
            ...
            throw new Error(`Failed to find all tasks for admin: ${error.message}`);
        }
    }
    ...
    static async update(id, updateData) {
        ...
        const sql = `
            ...
            SET title = COALESCE(?, title),
                ...
                hidden_from_user = CASE WHEN COALESCE(?, status) = 'completed' THEN 1 ELSE 0 END,
                updated_at = datetime('now')
            WHERE id = ?
        `;

        const params = [title, description, priority, due_date, status, status, id];
    ...
    static async updateStatus(id, status) {
        const sql = `
            UPDATE tasks 
            SET status = ?, 
                hidden_from_user = CASE WHEN ? = 'completed' THEN 1 ELSE 0 END,
                updated_at = datetime('now')
            WHERE id = ?
        `;

        try {
            await run(sql, [status, status, id]);
            return await this.findById(id);
        } catch (error) {
    ...
    static async getTaskCounts(userId) {
        const sql = `
            ...
            FROM tasks 
            WHERE assigned_to = ? AND (hidden_from_user = 0 OR hidden_from_user IS NULL)
        `;
